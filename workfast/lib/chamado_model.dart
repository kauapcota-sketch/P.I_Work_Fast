...
class Chamado {
  ...
  final File? imagem; // foto opcional anexada ao registrar o problema
...
class ChamadoService {
  // IMPORTANTE: não pode ser 'final' para permitir inserção
  ...
  /// Adiciona um novo chamado no topo da lista
  static void adicionarChamado(Chamado chamado) {
    _todosChamados.insert(0, chamado);
  }

  static List<Chamado> getChamadosPorCategoria(CategoriaChamado categoria) {
    if (categoria == CategoriaChamado.geral) {
      return List.from(_todosChamados);
    } else {
      return _todosChamados.where((c) => c.categoria == categoria).toList();
    }
  }

  static List<Chamado> get todosChamados => List.from(_todosChamados);
}
